...
// Demo traffic video used to simulate the live CCTV feed.
// Place demo_traffic.mp4 inside the `public/` folder of the project so it's
// served at this exact path by Vite (e.g. public/demo_traffic.mp4).
const DEMO_TRAFFIC_VIDEO_URL = "/demo_traffic.mp4";
...
export default function CctvSection({ onAddViolation, initialCameraId }: CctvSectionProps) {
  ...
  // Was: startVideoProcessing(true) -> requested navigator.mediaDevices.getUserMedia
  // (laptop webcam). Now loops the bundled demo_traffic.mp4 so the CCTV feed,
  // YOLOv8 tracking, ByteTrack IDs, and EasyOCR plate reads all run against the
  // same repeating footage instead of a live camera.
  const startCameraStream = () => startVideoProcessing(false, DEMO_TRAFFIC_VIDEO_URL);

  // Autoplay demo traffic video on mount
  useEffect(() => {
    startVideoProcessing(false, DEMO_TRAFFIC_VIDEO_URL);
